...
class SMSBalance(models.Model):
    """Singleton — only one row. Tracks owner's SMS credit balance."""
    credits = models.PositiveIntegerField(default=0, help_text="Number of SMS credits remaining")
    price_per_sms = models.DecimalField(
        max_digits=8, decimal_places=2, default=0,
        help_text="Price charged to owner per SMS (UGX). Owner sets this."
    )
    updated_at = models.DateTimeField(auto_now=True)
...
class SMSTopUp(models.Model):
    """Log of credit top-ups done by developers."""
    credits_added = models.PositiveIntegerField()
    amount_paid = models.DecimalField(max_digits=10, decimal_places=2, help_text="Amount owner paid (UGX)")
    ...
    def __str__(self):
        return f"+{self.credits_added} credits on {self.created_at.date()}"

    def save(self, *args, **kwargs):
        super().save(*args, **kwargs)
        balance = SMSBalance.get()
        balance.credits += self.credits_added
